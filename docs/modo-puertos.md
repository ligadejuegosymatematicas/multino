...
Al seleccionar una ficha, solo los valores compatibles adquieren halo y grosor adicional. La proyección compara scoring resultante, multiconjunto de extremos por valor, estado del ramificador y terminalidad para agrupar targets físicamente distintos con una consecuencia estratégica idéntica. El medallón prioriza la cantidad de decisiones y anota secundariamente cuántos lugares equivalentes representa cada una. Una sola decisión despacha un target canónico determinista; si hay efectos distintos, la UI los nombra por su consecuencia —por ejemplo, **Continuar**, **Completar cruce** o **Abrir brazo**— sin exponer `placementId` ni `portId`. No se muestran S futuro, divisibilidad ni puntos anticipados.

Esta separación deja preparada una convergencia futura, todavía no implementada, entre Puertos y Grafo: los mismos siete valores podrían formar una vista **Estratégico** con estados **Jugar** y **Analizar**, donde el segundo añade K₇, lazos y subgrafo jugado como overlays sin mover los medallones.
