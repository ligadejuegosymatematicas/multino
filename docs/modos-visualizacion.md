...
Puertos ocupa una posición conceptual intermedia: agrupa todas las apariciones del valor `n` dentro del único macro-nodo `B_n`, pero no colapsa qué incidencias continúan. En reposo funciona como vista estratégica: badge y halo teal expresan targets; anillo y centro dorados expresan exclusivamente `scoringTerms` y S. La topología de hilos, costuras y hubs reaparece en **Ver estructura**. La vista conserva targets exactos y participa del mismo feedback posterior de puntuación; no calcula S desde hilos o extremos. Su contrato completo está en [`modo-puertos.md`](modo-puertos.md).

En Tradicional, la geometría comunica la estructura: no se muestran permanentemente `P/A`, familias, contadores técnicos ni IDs. Los extremos libres son sockets próximos a la mitad abierta; la selección resalta compatibles y solo numera opciones concretas repetidas. Antes de completar la continuidad del ramificador, sus dos laterales se ven pegados a la ficha como sockets bloqueados y no son botones.

La cámara local ajusta y centra el contenido en la carga inicial, al cambiar realmente de orientación o por acción explícita, sin cruzar un tamaño mínimo legible. Una jugada ordinaria conserva zoom y pan; si la ficha nueva queda fuera de la zona segura, desplaza únicamente lo necesario para revelarla. Handoff, feedback, turno y terminalidad no provocan fit global. Si la mesa sigue siendo mayor, se recorre dentro de su viewport mediante tacto o arrastre, sin desplazar horizontalmente la página.
